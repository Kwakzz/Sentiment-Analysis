...
architecture = AutoModelForSequenceClassification.from_pretrained(original_model_name)
...
# collecting 1000 random samples
dataset_for_training = twitter_dataset["train"].shuffle(seed=42).select(range (1000))
dataset_for_testing = twitter_dataset["test"].shuffle(seed=42).select(range (1000))
